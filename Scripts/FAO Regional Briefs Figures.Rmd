...
# Set up the data with only aggregate values
  ...
  data <- FSCI_2024 %>%
    ...
    select(c(desirable_direction, year, country, value, short_label, indicator_order, 'UN Continental Region', globalmean, theme, unit)) %>% unique() %>%
...
# Reshape to table
  fortable <- fortable %>%
    select(year, short_label, theme, `Regional weighted mean`, `Global weighted mean`, unit, Region) %>%
