...
# Expert elicitation of global interactions results (datasets here are all based on the primary direct connections dataset, listed first, but then include the lowest connection, the direct connections shaped as a matrix (required for certain visualizations), and a dataset that preserves the information about bidirectional relationships identified)
...
Interactions_Direct_Matrix <- readRDS(file = file.path(data_out, "Interactions_ExpertElicit_DirectMatrix.rds"))
Interactions_Bidirect <- readRDS(file = file.path(data_out, "Interactions_ExpertElicit_Bidirect.rds"))

# Literature search dataset:
...
# Plot the estimates
  p1 <- all_results2 %>%
      ...
      theme_classic() +
      labs(y = "",
           x = "Average percent change over time (95% Confidence Interval)",
...
```{r, warning = FALSE, messages = FALSE, cache = TRUE}
  ...
  gov_litcounts <- gov_litcounts %>%
    ...
    # Bring in the organizing information
      themes_info1 <- Interactions %>% select(1,4,6) %>%
        rename(indicator_1 = from,
               theme1 = from_theme,
               order1 = from_order) %>%
        unique()
      themes_info2 <- Interactions %>% select(1,4,6) %>%
...
```{r, warning = FALSE, messages = FALSE, cache = TRUE}
 # Make the global mean into a row to visualize "as a country"
  ...
  # Add legend and save figure
    png(file.path(figtab_out, "SI Figure 3_Theme1 all data.png"), width = 7.5, height = 10, unit = "in", res = 300)
...
# Create an igraph object from the matrix of first order connections
  network <- igraph::graph_from_adjacency_matrix(Interactions_Direct_Matrix)
